...
export function CreatePoolForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const poolFactoryAddress = "0x8901234567890123456789012345678901234567" as `0x${string}`

      // Mock ABI for pool factory
      const poolFactoryABI = [
        {
          ...
          inputs: [
            ...
            { name: "commissionPercentage", type: "uint256" },
          ],
      ...
      const commissionBasisPoints = Math.floor(Number.parseFloat(formData.commissionPercentage) * 100)

      const hash = await writeContract({
        address: poolFactoryAddress,
        abi: poolFactoryABI,
        functionName: "createPool",
        args: [formData.name, formData.description, managerTypeValue, BigInt(commissionBasisPoints)],
      })
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Connect your wallet to create a new money management pool</CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col items-center justify-center py-8">
          <p className="mb-4 text-center text-muted-foreground">You need to connect your wallet to create a pool</p>
  ...
  return (
    <Card>
      <form onSubmit={handleSubmit}>
        <CardHeader>
          <CardTitle>Create a New Pool</CardTitle>
          <CardDescription>Set up your money management pool and start earning commissions</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          {txStatus === "error" && (
            <div className="rounded-md bg-red-500/10 p-4 text-red-500">Error creating pool: {txError}</div>
          )}
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) => handleSelectChange("managerType", value)}
              disabled={isTransactionPending}
            >
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              This is the percentage of profits you&apos;ll earn as the pool manager. Maximum 20%.
            </p>
          </div>
        </CardContent>
        <CardFooter>
          <Button type="submit" className="w-full" disabled={isTransactionPending}>
